...
namespace Raven.Client.Connection.Async
{
	...
	public interface IAsyncDatabaseCommands : IDisposable, IHoldProfilingInformation
	{
		...
		/// </summary>
		Task<Abstractions.Util.IAsyncEnumerator<RavenJObject>> StreamQueryAsync(string index, IndexQuery query, Reference<QueryHeaderInformation> queryHeaderInfo);

		/// <summary>
		/// Streams the documents by etag OR starts with the prefix and match the matches
		/// Will return *all* results, regardless of the number of itmes that might be returned.
		/// </summary>
		Task<Abstractions.Util.IAsyncEnumerator<RavenJObject>> StreamDocsAsync(Etag fromEtag = null, string startsWith = null, string matches = null, int start = 0, int pageSize = int.MaxValue);
