...
namespace Raven.Client.Shard
{
											   ...
											   IAsyncDocumentSessionImpl, IAsyncAdvancedSessionOperations
	{
		...
		public Task<AsyncDocumentSession.QueryYieldStream<T>> StreamAsync<T>(IAsyncDocumentQuery<T> query)
		{
		...
		public Task<Abstractions.Util.IAsyncEnumerator<StreamResult<T>>> StreamAsync<T>(IQueryable<T> query, Reference<QueryHeaderInformation> queryHeaderInformation)
		{
			throw new NotSupportedException("Streams are currently not supported by sharded document store");
		}

		public Task<Abstractions.Util.IAsyncEnumerator<StreamResult<T>>> StreamAsync<T>(Etag fromEtag, int start = 0, int pageSize = Int32.MaxValue)
		{
			throw new NotSupportedException("Streams are currently not supported by sharded document store");
		}

		public Task<Abstractions.Util.IAsyncEnumerator<StreamResult<T>>> StreamAsync<T>(string startsWith, string matches = null, int start = 0, int pageSize = Int32.MaxValue)
		{
			throw new NotSupportedException("Streams are currently not supported by sharded document store");
		}
