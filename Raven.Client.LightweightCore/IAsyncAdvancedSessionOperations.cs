...
namespace Raven.Client
{
	...
	public interface IAsyncAdvancedSessionOperations : IAdvancedDocumentSessionOperations
	{
		...
		/// </summary>
        Task<AsyncDocumentSession.QueryYieldStream<T>> StreamAsync<T>(IAsyncDocumentQuery<T> query);
		...
		/// Does NOT track the entities in the session, and will not includes changes there when SaveChanges() is called
		/// </summary>
		Task<Abstractions.Util.IAsyncEnumerator<StreamResult<T>>> StreamAsync<T>(IQueryable<T> query);
		...
		/// Does NOT track the entities in the session, and will not includes changes there when SaveChanges() is called
		/// </summary>
		Task<Abstractions.Util.IAsyncEnumerator<StreamResult<T>>> StreamAsync<T>(IQueryable<T> query, Reference<QueryHeaderInformation> queryHeaderInformation);
		...
		/// Does NOT track the entities in the session, and will not includes changes there when SaveChanges() is called
		/// </summary>
		Task<Abstractions.Util.IAsyncEnumerator<StreamResult<T>>> StreamAsync<T>(Etag fromEtag, int start = 0, int pageSize = int.MaxValue);
		...
		/// Does NOT track the entities in the session, and will not includes changes there when SaveChanges() is called
		/// </summary>
		Task<Abstractions.Util.IAsyncEnumerator<StreamResult<T>>> StreamAsync<T>(string startsWith, string matches = null, int start = 0, int pageSize = int.MaxValue);
	}
}